...
func (r *CatalogResolver) Search(ctx context.Context, ext *ocv1.ClusterExtension) ([]Bundle, error) {
	...
	if versionRange != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("desired version range %q is invalid: %w", versionRange, err)
		}
	}

	var searchResult []Bundle
	...
	if err := r.WalkCatalogsFunc(ctx, packageName, func(ctx context.Context, cat *catalogd.ClusterCatalog, packageFBC *declcfg.DeclarativeConfig, err error) error {
		...
		result := filter.Filter(packageFBC.Bundles, filter.And(predicates...))
		for _, b := range result {
			searchResult = append(searchResult, Bundle{Bundle: b, CatalogID: cat.Name})
		}
		return nil
	}, listOptions...); err != nil {
		return nil, fmt.Errorf("error walking catalogs: %w", err)
	}
	slices.SortStableFunc(searchResult, func(a, b Bundle) int {
		return compare.ByVersion(a.Bundle, b.Bundle)
	})
